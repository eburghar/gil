...
// NameOrId exists in gitlab crate but it doesn't implement FromStr
pub enum IdOrName {
	Id(u64),
	Name(String),
}
...
impl IdOrName {
	pub fn borrow<'a>(&'a self) -> NameOrId<'a> {
		match self {
			IdOrName::Id(id) => NameOrId::Id(*id),
			IdOrName::Name(name) => NameOrId::Name(name.into()),
		}
	}
}
